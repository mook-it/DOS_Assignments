defmodule Chord.Node do
  ...
  def handle_call(
        {:set_successor, new_succ},
        _from,
        {self_node_id, predecessor, finger_table, successor_list, m}
      ) do
    {_, finger_table} = Map.get_and_update(finger_table, 0, fn x -> {x, new_succ} end)
    {:reply, {:ok}, {self_node_id, predecessor, finger_table, successor_list, m}}
  end

  def handle_call(
        {:set_predecessor, new_pred},
        _from,
        {self_node_id, predecessor, finger_table, successor_list, m}
      ) do
    {:reply, {:ok}, {self_node_id, new_pred, finger_table, successor_list, m}}
  end
  ...
  def handle_call(
    ...
    my_succ = Map.get(finger_table, 0)
    my_pred = predecessor
    GenServer.call(:"node_#{my_pred}", {:set_successor, my_succ})
    GenServer.call(:"node_#{my_succ}", {:set_predecessor, my_pred})
    {:reply, {:ok}, {self_node_id, predecessor, finger_table, successor_list, m}}
  end
  ...
  def handle_call(
    ...
    {:reply, predecessor, {self_node_id, predecessor, finger_table, successor_list, m}}
  end
  ...
  def handle_call(
    ...
    {:reply, finger_table, {self_node_id, predecessor, finger_table, successor_list, m}}
  end
  ...
  def handle_cast(:stabilize, {self_node_id, predecessor, finger_table, successor_list, m}) do
    ...
    if(x != self_node_id) do
      ...
      {:noreply, {self_node_id, predecessor, finger_table, successor_list, m}}
    else
      {:noreply, {self_node_id, predecessor, finger_table, successor_list, m}}
    end
  end
  ...
  defp closest_preceding_node_helper(size_of_table, finger_table, key, self_node_id) do
    if(size_of_table == 0) do
      ...
      [{_, failed_nodes}] = :ets.lookup(:chord, "failed_nodes")
      size1 = Enum.count(failed_nodes)
      failed_nodes = failed_nodes -- [table_entry]
      size2 = Enum.count(failed_nodes)

      if(
        ((table_entry > self_node_id && table_entry < key) || key < self_node_id) &&
          size1 == size2
      ) do
        table_entry
      else
  ...
  def handle_call(
    ...
    {:reply, successor_for_key, {self_node_id, predecessor, finger_table, successor_list, m}}
  end
  ...
  def handle_cast(:fix_fingers, {self_node_id, predecessor, finger_table, successor_list, m}) do
